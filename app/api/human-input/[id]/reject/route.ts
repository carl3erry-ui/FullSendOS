...
import { z } from "zod";
import { rejectHumanInputRequest, getHumanInputRequest } from "@/services/human-input-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await rejectHumanInputRequest(id, body.response, body.resolvedBy);
    await recordAllow(actor, action, "human_input_rejected");
