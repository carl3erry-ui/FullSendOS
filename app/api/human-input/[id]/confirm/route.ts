...
import { z } from "zod";
import { confirmHumanInputRequest, getHumanInputRequest } from "@/services/human-input-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await confirmHumanInputRequest(id, body.response, body.resolvedBy);
    await recordAllow(actor, action, "human_input_confirmed");
