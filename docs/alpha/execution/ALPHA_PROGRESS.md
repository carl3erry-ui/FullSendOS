...
| --- | --- | --- | --- | --- | --- |
| 2026-07-28 | Engineering | ALPHA-052 | IN PROGRESS (ALPHA-052-03B COMPLETE) | Branch feature/alpha-052-03b-agent-task-approval-security | Implemented ALPHA-052-03B for POST /api/agent-tasks/[id]/approve, /reject, and /request-revision. Added authenticated actor requirement, stored task lookup, stored project ownership resolution, task-link integrity checks, internal-admin-only policy, deny-by-default internal_operator and client_user policy, best-effort security audit allow/deny events, safe generic 404 concealment for missing tasks, sanitized generic 500 handling, and safe response redaction of task payloads. Preserved existing overwrite semantics for approval state transitions and did not modify run-route or workflow-resume behavior. Validation after implementation: tsc pass (0 errors), targeted security route tests pass (55/55), security test suite pass (64/64), full tests pass (605/605), build pass with 10 Turbopack warnings (non-blocking baseline). Protected handlers: 10. Remaining route files: 36. ALPHA-052 remains IN PROGRESS. |
| 2026-07-27 | Engineering | ALPHA-052 | IN PROGRESS (ALPHA-052-03A COMPLETE) | Branch feature/alpha-052-03a-agent-task-run-security; starting main commit 755aee434ced25f9e8005b49e623d5c089e845b2 | Implemented ALPHA-052-03A for POST /api/agent-tasks/[id]/run only. Added authenticated actor requirement, stored task lookup, stored project ownership resolution, task-link integrity checks, internal-admin-only run policy, deny-by-default internal_operator and client_user policy, best-effort security audit allow/deny events, and safe success-response redaction (task/execution projections without prompt snapshots or raw provider fields). Preserved executor duplicate-run and approval gating behavior. Validation after implementation: tsc pass (0 errors), security-specific tests pass (43/43), targeted agent-task run tests pass (10/10), full tests pass (584/584), build pass with 10 Turbopack warnings (non-blocking baseline). Protected handlers: 7. Remaining route files: 39. ALPHA-052 stays IN PROGRESS. ALPHA-052-03B approve/reject/request-revision remains pending. |
