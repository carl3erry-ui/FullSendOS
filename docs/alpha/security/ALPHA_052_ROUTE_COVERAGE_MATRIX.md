...
| `/api/human-input/[id]/confirm` | `POST` | `FULL` | `internal_admin`, `internal_operator`, and `client_user` within stored request ownership scope | Stored request `clientId` + `engagementId` | `B1` | `PASS` | `feature/alpha-052-02-human-input-mutations`, `services/security-route-guards.test.ts` | `TBD` | Protected now |
| `/api/human-input/[id]/reject` | `POST` | `FULL` | `internal_admin`, `internal_operator`, and `client_user` within stored request ownership scope | Stored request `clientId` + `engagementId` | `B1` | `PASS` | `feature/alpha-052-02-human-input-mutations`, `services/security-route-guards.test.ts` | `TBD` | Protected now |
| `/api/human-input/[id]/skip` | `POST` | `FULL` | `internal_admin`, `internal_operator`, and `client_user` within stored request ownership scope | Stored request `clientId` + `engagementId` | `B1` | `PASS` | `feature/alpha-052-02-human-input-mutations`, `services/security-route-guards.test.ts` | `TBD` | Protected now |
...
| `/api/agent-tasks/[id]/run` | `POST` | `NONE` | Planned: internal admin/internal operator only within scope | Task ID + task store metadata | `B2` | `TBD` | `AgentExecutor`, provider registry | `TBD` | Pending ALPHA-052-03 |
| `/api/agent-tasks/[id]/approve` | `POST` | `NONE` | Planned: internal admin only or future reviewer role | Task ID + task store metadata | `B2` | `TBD` | `globalTaskStore.loadTask`, approval state update | `TBD` | Pending ALPHA-052-03 |
| `/api/agent-tasks/[id]/reject` | `POST` | `NONE` | Planned: internal admin only or future reviewer role | Task ID + task store metadata | `B2` | `TBD` | `globalTaskStore.loadTask`, rejection state update | `TBD` | Pending ALPHA-052-03 |
| `/api/agent-tasks/[id]/request-revision` | `POST` | `NONE` | Planned: internal admin only or future reviewer role | Task ID + task store metadata | `B2` | `TBD` | `globalTaskStore.loadTask`, revision request state update | `TBD` | Pending ALPHA-052-03 |
